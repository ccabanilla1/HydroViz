...
Future<Map<String, dynamic>> saveComponent(Map<String, dynamic> componentData) async {
    try {
      ...
      final mappedData = {
        ...
        'project': 1,  // Hardcoded for now till bug found to project 1 , pass from UI later
  ...
  Future<Map<String, dynamic>> getSimulationResults(String simulationId) async {
    try {
      ...
      _logger.severe('Error fetching simulation results: $e', e, stackTrace); // Consider using proper logging
      rethrow;
    }
  }
Future<void> deleteComponent(String componentId) async {
    try {
      ...
      final response = await http.delete(
        Uri.parse('$_baseUrl/components/$componentId/'),  // Note the trailing slash
        headers: _headers,
      );
