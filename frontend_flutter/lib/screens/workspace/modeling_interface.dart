...
import '../../widgets/workspace/component_palette.dart';
import '../../widgets/workspace/grid_painter.dart';
...
class ModelingInterface extends StatefulWidget {
  ...
  @override
  _ModelingInterfaceState createState() => _ModelingInterfaceState();
}
            ...
            child: Stack(
              children: [
                ...
                onAcceptWithDetails: (details) async {
                    try {
                      _logger.info('Drag details - offset: ${details.offset}');
                      
                      final newComponent = {
                        'type': details.data.name.toUpperCase(),
                        'location_x': details.offset.dx.toDouble(),
                        'location_y': details.offset.dy.toDouble(),
                        'properties': {},
                        'project': widget.projectId  // Use the passed project ID instead of hardcoding
                      };
                      
                      _logger.info('Component data being sent: $newComponent');
                      
                      final savedComponent = await _apiService.saveComponent(
                        newComponent,
                        widget.projectId  // Pass project ID to API service
                      );
